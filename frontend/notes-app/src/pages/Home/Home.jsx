  ...
  const deleteNote = async (data) => {
    ...
    try {
      ...
      if (response.data && !response.data.error) {
        showToastMessage("Note Deleted Successfully", "delete"), getAllNotes();
      }
    } catch (error) {
      if (
        error.response &&
  ...
  const updateIsPinned = async (noteData) => {
    ...
    try {
      ...
      if (response.data && response.data.note) {
        showToastMessage("Note Pinned Successfully"), getAllNotes();
      }
    } catch (error) {
      console.log(error);
    }
        ...
        {allNotes.length > 0 ? (
          ...
          <EmptyCard
            ...
            message={
              isSearch
                ? `Oops no notes found!`
                : `Start creating your first note! Click on the '+' button`
            }
          />
        )}
      </div>
